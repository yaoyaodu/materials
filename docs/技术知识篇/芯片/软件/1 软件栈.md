...
* Programming Model: **main concepts behind the MACA programming model**.
  ...
  * Heterogeneous Programming:
    * The MACA programming model assumes that the MACA threads (work-items) are executed on a physically separate device that operates as a coprocessor to the host running the C++ program. 
    * 代码都是跑在CPU上
  * Memory Hierarchy 
