...
<<<<<<< HEAD:docs/技术知识篇/芯片/软件/Python.md
## 正则表达式（特殊的字符序列）（re模块）
...
| 模式   | 描述                                                    |
| :----- | :------------------------------------------------------ |
| ^      | 匹配字符串的开头。``^a``只会去匹配字符串开头的a。如absorb。 |
| $      | 匹配字符串的末尾。``a$``只会匹配字符串末尾的a。如tea。     |
...
| 模式   | 描述           |
| :----- | :------------- |
| a*     | a出现0次或多次。``ab*c``*在这里代表b可以没有也可以出现多次，匹配ac、abc、abbbbc。表达式中间只能出现0或者多个b。|
| a+     | a出现1次或多次。``ab+c``匹配abc、abbbbc。 |
| a?     | a出现0次或1次，a这个字符可有可无。``used?``?代表前面的字符d需要出现0次或1次，匹配use、used。  |
| a{6}   | a出现6次。 ``ab{6}c``      |
| a{2,6} | a出现2到6次    |
| a{2,}  | a出现2次以上   |
| (ab)+  | ab出现1次或多次。匹配多个字符，用()括起来。   |

#### 或运算符(OR Operator)

| 模式       | 描述         |
| :--------- | :----------- |
| (a\|b)     | 匹配a或者b。``a (cat|dog)匹配a cat和a dog``   |
| (ab)\|(cd) | 匹配ab或者cd |
...
## 正则表达式（Regular Expression）（B站视频）
通配符的增强版，匹配指定规则的字符串。

测试正则表达式的工具：
* VS code搜索框的最右边，点选``.*``。
* 在线测试工具：http://regex101.com 

=======
while...else: 
>>>>>>> 2af23a7f97112cc48a4796497778d8596f1ad62b:docs/工具使用篇/Python办公自动化/Python10.9.md

* while条件：条件成立重复执行的代码
* else: 循环正常结束后要执行的代码

for循环用于针对序列中的每个元素的一个代码。
